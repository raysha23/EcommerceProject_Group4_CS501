...
const orderSummary = document.querySelector(
  "div.bg-white.p-6.rounded-lg.shadow.h-fit"
);
...
function renderCart() {
  ...
  cart.forEach((product, index) => {
    ...
    section.className =
      "bg-white p-6 rounded-2xl shadow-lg flex flex-col md:flex-row items-start gap-4 relative transition-all duration-300";
          ...
          <div class="mt-6 flex items-center justify-between border-t border-gray-300 pt-4">
            ...
            <div class="flex items-center gap-4 relative">
              ...
              <div class="relative z-[20]">
                <button class="qtyButton cursor-pointer flex items-center gap-1 sm:gap-2 border border-gray-300 rounded-lg px-2 py-1 sm:px-2.5 sm:py-1.5 md:px-3 md:py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-xs sm:text-sm md:text-sm">
                  <span class="qtySelected font-medium text-xs sm:text-sm md:text-sm">Qty: ${
                    product.quantity
                  }</span>
                ...
                <div class="qtyMenu absolute left-0 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 scale-95 transform transition-all duration-200 origin-top hidden z-[9999]">
                  <ul class="py-1">
                    ${[1, 2, 3, 4, 5]
                      .map(
                        (q) =>
                          `<li><button class="w-full cursor-pointer text-left px-3 py-1 hover:bg-gray-300" data-value="${q}">${q}</button></li>`
                      )
                      .join("")}
                  </ul>
                </div>
              ...
              <!-- Delete -->
              <button class="text-red-500 hover:text-red-600 p-2 rounded-lg hover:bg-red-50 deleteBtn cursor-pointer" data-index="${index}">
...
function updateSummary() {
  ...
  const totalPriceEl = orderSummary.querySelector("h3 span");

  // Animate total price change
  totalPriceEl.textContent = `$${totalPrice.toFixed(2)}`;
  totalPriceEl.classList.add("scale-110", "text-blue-600");
  setTimeout(
    () => totalPriceEl.classList.remove("scale-110", "text-blue-600"),
    300
  );
}
...
function setupQuantityButtons() {
  document.querySelectorAll(".qtyButton").forEach((button) => {
    ...
    menu.querySelectorAll("button").forEach((item) => {
      item.addEventListener("click", () => {
        ...
        const productIndex = deleteBtn
          ? parseInt(deleteBtn.dataset.index, 10)
          : NaN;
  ...
  document.addEventListener("click", () => {
    document
      .querySelectorAll(".qtyMenu")
      .forEach((menu) => menu.classList.add("hidden", "opacity-0", "scale-95"));
    document
      .querySelectorAll(".arrowIcon")
...
function setupDeleteButtons() {
  document.querySelectorAll(".deleteBtn").forEach((btn) => {
    btn.addEventListener("click", () => {
      deleteIndex = parseInt(btn.dataset.index, 10);
      showConfirmModal();
    });
  });
}
...
document.getElementById("confirmDelete").addEventListener("click", () => {
  if (deleteIndex !== null && !isNaN(deleteIndex)) {
